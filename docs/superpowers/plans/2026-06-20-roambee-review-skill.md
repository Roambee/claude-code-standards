# /roambee-review Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `/roambee-review` skill that reviews a git diff for Roambee standards violations and over-engineering patterns in one flat tagged list, and declare `ponytail` as an explicit plugin dependency.
...
### Task 2: Create the /roambee-review skill

**Files:**
- Create: `skills/roambee-review/SKILL.md`

**Interfaces:**
- Consumes: nothing (standalone skill)
- Produces: `roambee-review` skill registered and invocable as `/roambee-review` or `/roambee-review --staged`

- [ ] **Step 1: Create the skill directory and file**

Create `skills/roambee-review/SKILL.md` with the full content below. Copy exactly — the frontmatter, section headers, tag descriptions, examples, and output rules are all load-bearing.
...
```bash
head -8 skills/roambee-review/SKILL.md
```
...
```bash
git diff HEAD -- skills/roambee-review/SKILL.md | grep "^+" | head -20
```

Confirm the `+` lines are the additions you expect. Then invoke `/roambee-review --staged` in a Claude Code session and verify:
...
```bash
git add skills/roambee-review/SKILL.md
git commit -m "feat: add /roambee-review skill — combined standards + over-engineering diff review"
```
